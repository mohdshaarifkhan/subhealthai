    ...
    <div className="space-y-3 rounded-2xl border bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-gray-500">
          {data.day ? new Date(data.day).toDateString() : "Latest"}
        </span>
      </div>

      <ul className="space-y-2">
        {data.top_contributors.map((it, idx) => (
          <li key={`${it.feature}-${idx}`} className="flex items-center justify-between">
            <span className="text-sm">{prettyFeature(it.feature)}</span>
            <span
              className={`text-sm font-mono ${
                it.shap_value >= 0 ? "text-red-600" : "text-green-600"
              }`}
            >
