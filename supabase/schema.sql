...
CREATE OR REPLACE FUNCTION public.get_dataset_summary(p_version text)
RETURNS TABLE(dataset text, n_users integer, avg_steps numeric, avg_hr numeric, avg_hrv numeric, avg_rhr numeric, avg_sleep numeric)
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN QUERY
...
CREATE TABLE public.condition_risk (
    ...
    reasons jsonb,
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT condition_risk_risk_tier_check CHECK ((lower(risk_tier) = ANY (ARRAY['low'::text, 'moderate'::text, 'high'::text, 'critical'::text])))
);
...
CREATE TABLE public.device_accounts (
    ...
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT device_accounts_provider_check CHECK ((lower(provider) = ANY (ARRAY['fitbit'::text, 'oura'::text, 'garmin'::text, 'apple_health'::text, 'whoop'::text, 'samsung'::text, 'google_fit'::text]))),
    CONSTRAINT device_accounts_status_check CHECK ((lower(status) = ANY (ARRAY['active'::text, 'inactive'::text, 'error'::text, 'expired'::text])))
);
...
CREATE TABLE public.flags (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT flags_flag_type_check CHECK ((lower(flag_type) = ANY (ARRAY['sleep_debt'::text, 'hrv_decline'::text, 'rhr_elevated'::text, 'activity_instability'::text, 'stress_marker'::text, 'recovery_lag'::text]))),
...
CREATE TABLE public.risk_scores (
    ...
    CONSTRAINT risk_scores_confidence_check CHECK (((data_confidence IS NULL) OR ((data_confidence >= (0)::numeric) AND (data_confidence <= (1)::numeric)))),
    CONSTRAINT risk_scores_risk_score_check CHECK (((risk_score >= (0)::numeric) AND (risk_score <= (1)::numeric))),
    CONSTRAINT risk_scores_status_check CHECK ((status = ANY (ARRAY['computed'::text, 'insufficient_data'::text, 'stale'::text, 'error'::text])))
);
...
CREATE VIEW public.v_metrics_daily AS
 SELECT user_id,
  ...
  GROUP BY user_id, ((date_trunc('day'::text, event_time))::date), metric, source;
...
--
-- Name: idx_job_runs_type_started; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_job_runs_type_started ON public.job_runs USING btree (job_type, started_at DESC);
...
--
-- Name: ux_events_raw_dedupe; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_events_raw_dedupe ON public.events_raw USING btree (user_id, source, metric, event_time);


--
-- Name: metrics_extended trg_metrics_extended_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_metrics_extended_updated_at BEFORE UPDATE ON public.metrics_extended FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();


--
-- Name: metrics trg_metrics_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_metrics_updated_at BEFORE UPDATE ON public.metrics FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();


--
-- Name: model_config trg_model_config_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_model_config_updated_at BEFORE UPDATE ON public.model_config FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
ALTER TABLE ONLY public.allergies
    ADD CONSTRAINT allergies_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.condition_risk
    ADD CONSTRAINT condition_risk_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.family_history
    ADD CONSTRAINT family_history_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.labs_basic
    ADD CONSTRAINT labs_basic_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;
...
--
-- Name: metrics metrics_own; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY metrics_own ON public.metrics TO authenticated USING ((auth.uid() = user_id)) WITH CHECK ((auth.uid() = user_id));
